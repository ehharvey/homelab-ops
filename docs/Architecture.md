...
- Reads one `Instance`/`Network` definition (see Data model) and renders an IncusOS seed bundle: `install.yaml` (disk target, TPM/Secure Boot flags), `network.yaml` (static IP or DHCP), `applications.yaml` (`incus` only — no `operations-center` for v1)
...
- **Seed/installer generation** — same seed-rendering logic as the bootstrap CLI, generalized to any instance: builds `install.yaml`/`network.yaml`/`applications.yaml`, calls `flasher-tool`, and either serves the resulting `.img` for download or flashes it directly — whichever is easiest to implement first.
- **Cert issuance** — self-signed cert per node, for direct Incus client authentication (not Operations Center auth — that question is shelved with the OC-wrapping decision). Storage/rotation/revocation explicitly deferred.
