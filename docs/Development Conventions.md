...
Established by `internal/seed`'s `incus.yaml` rendering and `scripts/validate/node-boots-and-trusts-bootstrap-cert.sh`:
...
- **Validation scripts should drive the real stack, not just check that files exist or that unit tests pass.** `scripts/validate/node-boots-and-trusts-bootstrap-cert.sh` drives the full pipeline — `gen-cert` → `render-seed` → `build-image` → boot a real Incus VM off the produced `.img` → poll the node's static IP over HTTPS with the cert — as one end-to-end check that proves both install success and cert trust simultaneously. This caught real bugs a file-existence check would have missed (see below); prefer this pattern whenever there's a real artifact or service in the loop. Requires `INCUSOS_BASE_IMAGE`; skips that section gracefully when unset. A real VM boot is one instance of this principle, not the only one — `scripts/validate/store-retains-synced-fleet.sh`/`sync-warns-on-config-diff.sh` apply the same idea to the web app: drive the real `docker compose` stack against a real git remote, pushing a second commit to it to prove store-replace and diff behavior, rather than relying on `go test` alone.
...
- `Makefile` targets: `build`, `test` (`-race -cover`), `lint`, `lint-docs`, `validate`, `validate-hardware`, `fmt`, `tidy`, `clean`, `hooks`, `ship`, `lgtm`. Plain Make — no justfile/task-runner, since Make is already in the devcontainer base image.
- **`make validate` runs the validation suite's unattended subset** (`scripts/validate/`, ~2.5 minutes, Docker only); `make validate-hardware` runs the Incus/VM subset, which boots real VMs and needs the host. Both go through `scripts/validate/run.sh`, which discovers scripts from their own `VALIDATE_GROUP` declarations rather than a list — so a new script is covered without editing anything central, the same reasoning as `lint-mermaid.sh` globbing `docs/*.md`. An unmet prerequisite is a **skip with its own exit code**, not a failure: see `scripts/validate/README.md` and `docs/Decisions.md` §20.
