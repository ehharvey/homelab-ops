...
- **Validation scripts should boot something real, not just check that files exist.** `scripts/validate-issue-5.sh` (`make validate-issue-5`) drives the full pipeline — `gen-cert` → `render-seed` → `build-image` → boot a real Incus VM off the produced `.img` → poll the node's static IP over HTTPS with the cert — as one end-to-end check that proves both install success and cert trust simultaneously. This caught real bugs a file-existence check would have missed (see below); prefer this pattern over assertion-only scripts whenever there's a real binary/image artifact in the loop. Requires `INCUSOS_BASE_IMAGE`; skips that section gracefully when unset.
