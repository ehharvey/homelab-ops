...
// instances, added/changed/removed within each. Returns nil if Empty().
...
// pulled state), matching Networks/Instances by Name. A name present in
// both is "changed" if its full struct value differs (reflect.DeepEqual);
